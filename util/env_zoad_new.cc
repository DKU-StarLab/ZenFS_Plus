...
namespace leveldb {
    ...
    namespace {
		...
		read_buf_ = new char[buf_size[flag]];
		read_buf_offset_ = 0;
		read_buf_pos_ = 0;
            }
		...
		//unsigned int read_buf_size = n/IO_SIZE;
		/*
		if(n%IO_SIZE){
			read_buf_size++IO_SIZE;
		}
		*/
		...
		//else if(offset > file_size - buf_pos_){
  		...
  		//std::cout << "ZFR0.5 " << std::endl;
			/*
		    if(offset >= read_buf_offset_ and offset + n <= read_buf_offset_ + read_buf_pos_){
		        memcpy(scratch, read_buf_ + offset - read_buf_offset_, n);
		    }
		    else if(offset >= read_buf_offset_ + read_buf_pos_ or offset < read_buf_offset_){
                        
			z_load(uid, lp_id[flag], key, read_buf_, offset, read_buf_size);
			
			read_buf_offset_= (offset/IO_SIZE) * IO_SIZE;
			read_buf_pos_ = read_buf_size;
			
			memcpy(scratch, read_buf_ + offset - read_buf_offset_, n);
		    }
		    else{
			memcpy(scratch, read_buf_ + offset - read_buf_offset_, read_buf_offset_ + read_buf_pos_ - offset);
			z_load(uid, lp_id[flag], key, read_buf_, read_buf_offset_ + read_buf_size, read_buf_size);
			memcpy(scratch, read_buf_ , n - (read_buf_offset_ + read_buf_pos_ - offset));
			read_buf_offset_= ((read_buf_offset_ + read_buf_size)/IO_SIZE) * IO_SIZE;
			read_buf_pos_ = read_buf_size;
		    }*/
		...
		*/
        ...
        public:
    		...
    		//std::cout << "ZW" << std::endl;
                ...
                ZoadFile* file = new ZoadFile(0, level, fname, global_key);
    		...
    		//std::cout << "ZA" << std::endl;
                ...
                if (file == nullptr) {
                    global_key_lock.Lock();
                    file = new ZoadFile(0, level, fname, global_key);

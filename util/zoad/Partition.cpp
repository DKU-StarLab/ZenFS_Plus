...
Partition::Partition(std::string p_name, int izg_num, unsigned int p_flag){
    ...
    mk_flag = p_flag;
    *status = CLOSE;
    read_buf = new char[IO_SIZE * izg_cnt];
    mtx = new std::mutex;
...
void Partition::del(){
    ...
    status_lock->lock();
    if(*status != CLOSE){
        std::cout << "Delete Open Zone" << std::endl;
        exit(0);
    }
    //Reset Zones, delete map
    status_lock->unlock();
    delete io_queue;
    delete io_queue_lock;
    delete status;
...
std::vector<class Zone*>::reverse_iterator Partition::get_zone_last_iter(){
    return zone_vector->rbegin();
}
std::vector<class Zone*>::reverse_iterator Partition::get_zone_reverse_first_iter(){
    return zone_vector->rend();
}
